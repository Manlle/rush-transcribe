...
def _run_whisper(tmp_path: str, lang: str):
    ...
    return {"text": text or "", "language": info.language, "duration": round(info.duration, 1)}


@app.post("/transcribe_url")
async def transcribe_url(url: str = Form(...), lang: str = Form("auto")):
    # The app sends just the audio URL; the server downloads + transcribes it.
    # This avoids any file-system handling on the phone.
    try:
    ...
    try:
        return _run_whisper(tmp_path, lang)
    except Exception as e:
